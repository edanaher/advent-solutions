...
# For all other lines, just start computing
2,$ {
  s/^/\&start /
}
...
:begin
/^\&start / {
  # Append the current state and table
  G;
  s/\
/;/
  # And munge the input into code to compute the side lengths
  s/\&start \([0-9]*\)x\([0-9]*\)x\([0-9]*\)/\&sides \&mul(\1,\2) \&mul\(\1,\3\) \&mul\(\2,\3\)/
}

/^\&sides [0-9]* [0-9]* [0-9]*;/ {
  s/^\&sides \([0-9]*\) \([0-9]*\) \([0-9]*\);\([0-9]*\);/\&add(\&mul(2,\&add(\1,\2,\3)),\&min(\1,\2,\3),\4);/
  #p
}
...
1 {
  ...
  /&buildmulttable 100/ {
    # Clean out the header so only the table and start value (0) is left
    s/&buildmulttable 100[^;]*;/0;/
    # Now switch the original input back in and add the start command!
    x
    s/^/\&start /
    #p
  }
...
/&multiplyacc(,[0-9]*,0*\([0-9]*\))/ {
  s/&multiplyacc(,[0-9]*,0*\([0-9]*\))/\1/
  #p
}

# Convenience for multiple add
/\&add(\([0-9]*\),\([0-9]*\),\([^)]*\))/ {
  s/\&add(\([0-9]*\),\([0-9]*\),\([^)]*\))/\&add(\&add(\1,\2),\3)/
  #p
}
...
# Convenience for multiple min
/\&min(\([0-9]*\),\([0-9]*\),\([^)]*\))/ {
  s/\&min(\([0-9]*\),\([0-9]*\),\([^)]*\))/\&min(\&min(\1,\2),\3)/
  #p
}
...
/&minaux([0-9]*,[0-9]*,[0-9]*,[0-9]*)/ {
  ...
  s/&minaux(\([1-9]\)\([0-9]*\),\([1-9]\)\([^)]*)\)/\&minaux(\&dec(\1)\2,\&dec(\3)\4/
  #p
}
...
# And we're done with this line; save what we've got for the next line.
h

# If we're on the last line, clean up the output; otherwise, delete it.
$ s/;.*//
$! d
